...
Every football match in Sweden for the coming week on one map, from Allsvenskan down to a Tuesday-night P12 game on the local turf. Switch between men's and women's football (men by default), then filter by senior/youth/kids/vets & rec, league tier (1–9+, C for Svenska Cupen, R for reserves), national or district series and age group, then hit **Near me** to see what's being played around you.
...
Svenska Cupen is tier **C**. Reserve, B-team and development leagues are tier **R**. Sweden's national teams have their own blue-and-gold **SWE** chip. Friendlies, qualifiers, district championships (DM) and other cups fall under **Other**, along with youth, kids' and veterans' games. Map markers show the highest tier being played at each venue.
...
- **Fixtures** come from the JSON endpoint behind [Matcher idag](https://www.svenskfotboll.se/serier-cuper/matcher-idag/) (`/api/matches-today/games/?associationId=…&date=…`). The script queries SvFF plus all 24 district associations for today and the next 13 days (`DAYS=14`): 350 small requests a day. District and youth games are rarely scheduled further out than two weeks. A typical week has about 10,000 matches. The match detail pages sit behind a Cloudflare challenge, so only this API is used.
...
- **Club crests** are downloaded once per club (`scripts/lib/crests.mjs`), refreshed every 60 days, kept in the Actions cache and published with the site, so visitors never load images from svenskfotboll.se. The list and popups show them for every team, and at city zoom venues hosting a tier 1–4, national-team or Svenska Cupen game get a fixture label with the home club's crest.
